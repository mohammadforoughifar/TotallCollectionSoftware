-- =============================================================
-- اسکریپت کامل راه‌اندازی دیتابیس انبار (InventoryDb)
-- اجرا در SSMS: کل فایل را باز کرده و Execute (F5) بزنید.
-- ایمن (idempotent): اجرای مجدد مشکلی ایجاد نمی‌کند.
-- =============================================================
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260817000000_AddSystemInfo'
)
BEGIN
    CREATE TABLE [SystemInfos] (
        [Id] int NOT NULL IDENTITY,
        [AgentId] nvarchar(50) NOT NULL,
        [Motherboard] nvarchar(200) NULL,
        [Cpu] nvarchar(200) NULL,
        [Ram] nvarchar(100) NULL,
        [HardDisk] nvarchar(200) NULL,
...
BEGIN
    CREATE INDEX [IX_SystemInfos_AgentId] ON [SystemInfos] ([AgentId]);
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260817000000_AddSystemInfo', N'8.0.0');
END;
GO
