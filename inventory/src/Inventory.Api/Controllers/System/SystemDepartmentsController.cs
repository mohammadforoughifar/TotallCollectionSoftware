...
public class SystemDepartmentsController : ControllerBase
{
    ...
    [HttpPut("{id}")] public async Task<ActionResult> Put(int id, [FromBody] SystemDepartment d) { var item = await _db.SystemDepartments.FirstOrDefaultAsync(x => x.Id == id); if (item == null) return NotFound(); item.Name = d.Name; item.Code = d.Code; item.CompanyId = d.CompanyId; item.IsActive = d.IsActive; await _db.SaveChangesAsync(); return Ok(); }
