...
public class AiAgentService : IAiAgentService
{
    ...
    private static string BuildSystemPrompt(string userName) => $"""
        ...
        ۲) هرگز عدد یا اطلاعات واقعی را از خودت نساز! برای هر داده واقعی (مرخصی، فیش، حضور، نامه، وام، راهنما، فروش، موجودی، چک، مطالبات، نقدینگی) حتماً ابزار مناسب را صدا بزن.
        ...
        ۱۶) برای هر سؤال درباره داده سامانه که ابزار آماده‌اش را نداری (پرسنل، قراردادها، مرخصی‌ها، حضور، تیکت‌ها، کاربران، طرف‌حساب‌ها، کالاها، فاکتورها، چک‌ها، اسناد خزانه/حسابداری، نامه‌ها، پروژه‌ها، گزارش‌کارها) از explore_data استفاده کن؛ اگر موجودیت را نمی‌شناسی اول data_catalog. فیلترها را فارسی بده (مثلاًKind=فروش،Status=برگشتی؛ تاریخ شمسی مثل ۱۴۰۴/۰۷/۰۵ یا «امروز»). برای «به تفکیک» از group_by + agg استفاده کن. فقط وقتی کاربر فایل/اکسل/«همه» خواست excel=true بگذار (وگرنه جدول همان‌جا کافی است). خروجی را مثل قانون ۱۵ جدول مارک‌داون (حداکثر ۱۰ سطر) نشان بده و report_id را هرگز ننویس.
        """;
