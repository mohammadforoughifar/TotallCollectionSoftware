...
| **RadisHr V019** (منابع انسانی جامع) | `Controllers/RadisHr` (زیر `/api/hr`؛ alias قدیمی `/radis-hr/api`) | `Inventory.Shared/RadisHr/Models`؛ DbContext و مایگریشن در `Data/RadisHr` | `Services/RadisHr` | `Pages/RadisHr` داخل Inventory، مسیر `/hr` و همان منو/نشست/قالب |
