...
{
    // Inventory's WASM client uses InvariantGlobalization (no ICU download).
    // Format explicitly so HR never throws CultureNotFoundException during first render.
    private static string PersianNumber(string value) =>
        ToPersianDigits(value).Replace(',', '٬').Replace('.', '٫');
    ...
    public static string N(decimal value) => PersianNumber(value.ToString("#,##0", CultureInfo.InvariantCulture));
    public static string N(int value) => PersianNumber(value.ToString("#,##0", CultureInfo.InvariantCulture));
    public static string N(double value) => PersianNumber(value.ToString("#,##0.##", CultureInfo.InvariantCulture));
    ...
    public static string Percent(double value) => $"{PersianNumber(value.ToString("0.#", CultureInfo.InvariantCulture))}٪";
    public static string Percent(decimal value) => $"{PersianNumber(value.ToString("0.##", CultureInfo.InvariantCulture))}٪";
